// Vercel serverless webhook for Stripe payment events (jevline)
...
const GH_REPO = process.env.GH_DATA_REPO || 'jevline-data';
const MAIL_FROM = process.env.JEVLINE_MAIL_FROM || 'gentledesk632@agentmail.to';
const AGENTMAIL_KEY = process.env.AGENTMAIL_API_KEY || '';
const BUYERS_FILE = 'buyers.json';
const USERS_FILE = 'users.json';
const ACCESS_PEPPER = process.env.ACCESS_PEPPER || 'jevline-pepper-sse9wpkl';

function genCode() {
  const chars = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
  let s = '';
  for (let i = 0; i < 10; i++) s += chars[crypto.randomInt(chars.length)];
  return `JL-${s.slice(0, 5)}-${s.slice(5)}`;
}
...
async function sendEmail(buyer, code) {
  ...
  const body =
`Welcome to Jevline™ — The Typed Decision Playbook ✅

Hi ${buyer.name || 'there'},

Thank you! Your order is confirmed.

YOUR PERSONAL ACCESS CODE: ${code || 'JL-XXXXX-XXXXX'}

OPEN YOUR DOWNLOADS:
→ https://jevline.vercel.app/download.html
(enter your email + access code above)

WHAT'S INSIDE (8 deliverables):
• 1. The Typed Decision Playbook (core guide: the ASK→SHAPE→CALIBRATE→GATE loop)
...
• 4. The Abstention & Escalation Design (ALLOW / REVIEW / REFUSE, never a bare yes/no)
• 5. The Cascade & Cost Blueprint (cheap judgment first, expensive thinking second)
• 6. The Decision Ledger + Drift Watch (log every judgment, catch drift first)
...
1. Pick ONE branching step currently doing a string match.
2. Write the STATE + ONE Noul question whose answer your code can use.
3. Call it once and LOG the probability — wire nothing yet.
4. Label ten real cases and check what a 0.9 threshold would have done.
5. Only then add the gate: allow / review / refuse.

Your LLM writes the words. A typed decision gives it a judgment you can test.
...
const res = await fetch(`https://api.agentmail.to/v0/inboxes/${MAIL_FROM}/messages/send`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${AGENTMAIL_KEY}` },
    body: JSON.stringify({ to: [buyer.email], subject: "Your Jevline™ Typed Decision Playbook access ✅", text: body }),
  });
  return res.ok;
}
